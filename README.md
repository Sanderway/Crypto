...
- 终端展示时附带两段 prompt：
  ...
  - `ark_prompt`：可直接投喂火山引擎 Ark 模型（已限定 JSON 输出格式）。
...
如需持续轮询并把分析交给 Ark 模型 + Telegram 推送：

```bash
python main.py --watch --refresh-seconds 120 \
...
- `--watch` 与 `--refresh-seconds`：开启轮询与轮询周期（秒），便于近似实时监测。
